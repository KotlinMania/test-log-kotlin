# Claude Code Project Instructions — test-log-kotlin

## Project Overview

This is **test-log-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`test-log`](https://crates.io/crates/test-log).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/test-log/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/test-log>
