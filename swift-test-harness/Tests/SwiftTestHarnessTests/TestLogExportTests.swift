...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TestLog.swiftmodule/`
...
//      `import TestLog` above would fail at compile time.
//
//   2. The static archive `libTestLog.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TestLog" }` and
//      `flattenPackage = "io.github.kotlinmania.testlog"` configuration in
...
//      via the `TestLogLibrary` product.
//
...
final class TestLogExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TestLog swift module imported cleanly")
    }
}
