...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`test-log`](https://crates.io/crates/test-log).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `test-log` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

`#[test_log]` test-attribute that captures log output
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:test-log-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `test-log` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
